...
export class InstanceViewComponent implements OnInit, OnDestroy {
  ...
  loadInstance(dbId: number,
    ...
    // Reloading the instance already on display is a refresh in place - a post-edit notification,
    // a display name regenerated by an auto-filler - not a switch to something else. The spinner
    // must not be shown for it: it *ngIf's the attribute table out of the DOM (see the template),
    // and the table is the element that scrolls, so remounting it drops the curator back at the
    // top of a long instance after an edit. Whenever the re-fetch takes even one tick this is
    // visible, and it also leaves this.instanceTable undefined for the duration (see
    // _loadIntance). Only cover a real switch, where starting at the top is what is wanted, and a
    // comparison load, which always goes to the server for the database copy.
    const isRefreshInPlace = !needComparsion && this.instance !== undefined && this.instance.dbId === dbId;
    setTimeout(() => {
      if (!isRefreshInPlace)
  ...
  private _loadIntance(instance: Instance,
    ...
    this.runInstanceViewFilters(instance).subscribe(filteredInstance => {
      ...
      // The table is absent while the spinner covers a switch, so it cannot be assumed to be here.
