### Build on September 21, 2026

- Bug fix: for a reaction whose input or output list already had two or more different entries when it was first created (for example, a reaction added with two distinct inputs), correcting a stoichiometry count - via **Auto-Fix**, or by hand - could look right while editing, then lose its "×N" label the moment editing was turned off, even though the corrected count was still saved correctly and re-running **Validate Diagram Against Database** showed no problem. The branching point those inputs or outputs originally shared was missing a marking the diagram needs to decide whether to draw the count at all; toggling editing off carried that gap forward into the redrawn arrow every time. The stoichiometry label is now drawn correctly after editing is disabled, including on reactions already affected by this before today.

- Bug fix: saving a pathway diagram containing a drug (a ChemicalDrug or ProteinDrug used as a therapeutic) or a Cell could quietly record it under its plain, non-drug type - or as a generic entity - once the diagram was uploaded, even though it still looked and behaved correctly on screen right up until the save. Drugs and Cells are now saved with their correct, specific type.

- Added: creating a new InstanceEdit directly - via "Create a New Instance" or by choosing "Add via Creation" for a slot that takes one - now automatically stamps its dateTime with the current time (GMT), the same as an InstanceEdit created automatically when you commit an edit. Its display name is generated right away instead of being left blank until dateTime is filled in by hand.
