...
- Bug fix: the **Deleted** instance created when you commit a deletion was being saved under the placeholder name "To be generated" rather than a real one, since its name is only generated when an attribute is edited and the creation dialog fills in the deleted dbIds for you. Six such Deleted instances are in the database. It is now named before it is committed - as **"Deletion of instance: 12345"**, or **"Deletion of instances: 12345, 67890"** when several instances are deleted together, matching how every other Deleted instance in the database is named. Deleting several instances at once previously would have used the singular wording.
