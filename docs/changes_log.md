...
- Bug fix, worth reviewing recent batch edits: a batch **Set** / **Replace** on an attribute whose values are instances changed *every* instance in the batch, not only the ones actually holding the value picked in the "Values of ... for selected instances" dialog. On a multi-valued attribute it also always replaced the **first** value rather than the one picked. Text and number values were matched correctly - only instance values were affected, because they were compared in a way that makes any two instances look identical. Instance values are now matched by dbId, so a replace lands only on the instances, and the value, you selected. If you have used batch Set/Replace on a slot such as `species`, `compartment` or `referenceEntity`, it is worth reviewing those staged edits before committing them.
