...
- **Validate Diagram Against Database** now checks reaction structure and display names against the diagram you actually have open, rather than the last copy uploaded. Three things follow from that. **Auto-Fix**'s corrections are reflected the moment you re-run validation, so you no longer have to upload first to see whether a fix took effect - this replaces the reminder described in the September 1 build below, which now says instead that the fix won't be part of the next release until you upload. A brand-new pathway diagram that has never been uploaded can now be validated at all, where before there was no saved copy to check it against. And compartments you added yourself while editing are now included in the compartment name check, not only the ones that came from the published diagram. Because the report now describes your open diagram rather than what is released, a note after a clean validation reminds you to upload if you have unsaved changes.

- Bug fix: a catalyst or regulator drawn with bend points - its line broken into segments through intermediate points instead of running straight to the reaction - was invisible to **Validate Diagram Against Database**, which reported the catalyst or regulator as missing even though it was plainly drawn, and **Auto-Fix** then "corrected" it by adding a second, duplicate link to the same entity. A connector's role now travels with each of its segments, so a bent catalyst or regulator line is recognized as what it is. Relatedly, when validation removes a connector that genuinely does not belong, it now removes the whole line including its bend points, instead of leaving orphaned points attached to the reaction and leading nowhere.

- Bug fix: another way the same duplicate-link symptom could appear - while the details of a catalyst or regulator were still being loaded, validation could not yet tell which entity it pointed at, treated the link already drawn for it as not belonging, and removed it, only to add it back as a brand new link in a different position on a later run. Validation now leaves the reaction alone until every participant has finished loading.

- Bug fix: a stoichiometry shown on a catalyst or regulator link (the count displayed when the same entity takes part more than once) was cleared every time **Validate Diagram Against Database** ran, even when it had been correct. Where a reaction records the same catalyst or regulator through more than one entry, those are now counted together instead of being handled repeatedly.

- Bug fix: releasing a diagram's lock from anywhere other than the open diagram itself - most often the lock list in the **Status** panel, which can unlock any diagram without it being open - left the editor still fully editable, so you could go on editing a diagram whose lock you no longer held. Its periodic background backups also kept firing and being rejected by the server. Editing now stops as soon as the lock is gone, whether it was released elsewhere by you or taken over by another window, and the diagram returns to the published version - once the lock is gone there is no guarantee your in-progress edits are still kept anywhere, and someone else may already be editing their own version.

- Bug fix: **Unlock** left the diagram visibly editable - nodes could still be dragged - until a background backup finished, even though it had already been treated as no longer being edited. Unlocking now stops editing immediately.

- Bug fix: advanced search over your staged instances (the list titled *Class*@Staged, holding your new, updated and deleted instances) returned instances that have no value at all in the attribute being searched. **Not Equal**, **Contains** and **Regex** each treated a missing value as empty text, and empty text satisfies all three, so the same conditions returned more instances from your staged changes than from the database, which excludes them. A missing value now matches **IS NULL** and nothing else, and an attribute holding an empty list of values counts as missing - previously it matched nothing at all, including **IS NULL**. **Not Equal** on a multi-valued attribute now means none of the values match, the same as it does against the database, rather than at least one: an instance whose values are `a` and `b` no longer comes back for **Not Equal a**. Separately, while staged changes were still loading, the search could evaluate every condition against a blank value and return the wrong instances; it now waits for the values it needs.
