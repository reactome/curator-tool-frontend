### Build on September 14, 2026

- Bug fix: adding an event to a pathway's **hasEvent** while the event view was open sometimes did not show up in the event tree, and from then on nothing else you edited showed up there either - events added, events removed, and edits you undid all left the tree exactly as it was, until you reloaded the page. This happened when the edit put an event inside something that already contains it: a circular relationship like that cannot be drawn as a hierarchy, and the tree gave up building itself and kept showing what it had before, with no message to say so. A circular relationship created this way is now reported the moment you make it - the same **Circular Reference in the Event Hierarchy** message you get when opening the event view, naming the containment path and the event to remove it from - and the rest of the tree updates as usual, including whatever else you added in the same edit. As before, the edit itself is not refused: it stays staged, and the hierarchy simply leaves that one relationship out until you remove it.

### Build on September 11, 2026

- Changed: adding an event to a pathway's **hasEvent** is no longer refused when that pathway already contains it. The check that refused it had to work out what already contained the event, which meant asking the server about every pathway above it on every single edit - slow on an ordinary edit, and hundreds of requests for a batch edit over a list - and if any of those requests failed the edit was refused outright, so a moment's trouble reaching the server blocked curation that was perfectly fine. Such an edit now simply goes through, and you are told about it in the **event view** instead, which is the one place where the hierarchy has to be a hierarchy: opening it reports each circular relationship, names the containment path (e.g. *"Metabolism [10] > Glycolysis [20] > Metabolism [10]"*) and says which event to remove it from. The rest of the tree loads as usual, so you can navigate straight to that event and fix it. Relationships you have created but not yet committed are reported too, and marked as such. Still refused at the point of editing, because it costs nothing to check: putting an event directly into its own **hasEvent** or **precedingEvent**. If the event hierarchy cannot be loaded at all, a circular reference is now named as the likely cause rather than leaving you with an empty tree and no explanation.

### Build on September 10, 2026

- Bug fix: the circular-reference check added on September 8 did not catch every case. Adding an event to a pathway's **hasEvent** was only checked for containment if the event tree had already been loaded in that session - so an edit made from the schema view (find the pathway in a list, open it, add to `hasEvent`) was allowed through, and a pathway could be put inside itself in a single edit. Once such a relationship was committed, the event hierarchy stopped loading altogether for everyone, because it is built in one pass that followed `hasEvent` round the loop until it gave up - which also meant there was no event tree left to find and remove the relationship through. Containment is now established by looking at the events above the one being edited, which works the same way wherever you are editing, and a hierarchy that already contains a loop now loads with the offending relationship left out (and logged on the server) instead of not loading at all. If the check cannot be completed - the server could not be reached, say - the edit is refused with an explanation rather than allowed unchecked.

### Build on September 8, 2026

- Bug fix: logging out and logging back in as a different curator, without reloading the page, left the previous curator's pathway diagram locks in place: the status panel's lock list still showed their diagrams, the lock icon appeared on a diagram the new curator had never locked, and enabling editing on such a diagram went ahead without checking with the server first - so two curators could be editing the same diagram, each believing they held the lock. The list of locks you hold is kept in the browser to avoid asking the server for it over and over, and logging out did not clear it, because logging out only returns you to the login page rather than reloading the app. That list is now tied to the curator it was fetched for and is discarded the moment the signed-in user changes, so a new session always starts from the locks the server says you actually hold. Logging out on its own clears it too, and a session left to time out is treated the same way.
...
- Added: an event can no longer be put inside itself. Adding an event to a pathway's **hasEvent** is now refused if that event *is* the pathway being edited, or if it already contains it further down - a **Circular Reference** message names the chain, e.g. *"Metabolism [10] already contains Glycolysis [20] (Metabolism > Glycolysis)"* - and the slot is left untouched. The same applies to an event added to its own **precedingEvent**. A hierarchy that contains itself has no meaning and hangs the event tree, which is why this is refused rather than warned about. Note that a preceding-event *loop* between two or more different events is still allowed: a kinase and the phosphatase that reverses it legitimately precede each other, and hundreds of events in the database are in such a loop by design. In a batch edit the instances that would have been made to contain themselves are left unchanged and counted in the summary, so the rest of the batch still goes through.
...
- Bug fix: adding a value to a slot near the bottom of a long instance - a summation or the species of a pathway, say - jumped the attribute table back to the top, so you had to scroll down again to carry on with the lower slots. After every edit the table was rebuilt by discarding all of its rows and creating them again from scratch; on an instance with enough attributes to scroll, the browser sees the table empty for an instant during that and resets the scroll position to the top. The table still rebuilds its rows the same way, but now remembers where you were scrolled to and puts you back there afterwards, rather than leaving you at the top. Opening a different instance still starts at the top.
