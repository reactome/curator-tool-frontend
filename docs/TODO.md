...
- Add a check for event tree to see if there is any circular reference (high important)
...
- Log-in (most important); how to handle the writing privledges  

- Merge two instances (Done: matched instances in the duplicate-check dialog, and any two instances via the merge_type button in the instance view - either into a new third instance with per-value picking, or one instance merged into the other)

- TODO: Enable users to change the password. The password generated by us should expire after the first time use.
- TODO: Enable to support specifial characters (e.g. @,#,$,%,&,*) in the passwords. (lowest priority)
...
- QA check for a staged instance from Reactome team meeting 8/4 
